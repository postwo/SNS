...
@Entity
@Table(name = "\"users\"",
indexes = {@Index(name = "user_username_idx",columnList = "username",unique = true)}) //이렇게 하는이유는 postgres sql을 사용하고 있는데 postgres sql내부에서 이미 사용되고 있는 예약어 이다 그래서 이렇게 해줘야 데이터베이스 내부적으로 사용가능하다
//unique = true) 중복생성을 사용못하게 막는다
@Getter
@Setter
...
public class UserEntity implements UserDetails {
    ...
    public static UserEntity of(String username,String password){
        ...
        userEntity.setProfile("https://avatar.iran.liara.run/public/"+ (new Random().nextInt(100))+1); //1~100까지 범위
